...
class Medgan(object):
    ...
    def buildAutoencoder(self, x_input):
        ...
        return loss, decodeVariables, x_reconst
    ...
    def generateData(self,
        ...
        _, decodeVariables, x_reconst_ae = self.buildAutoencoder(x_dummy)
    ...
    def train(self,
        ...
        loss_ae, decodeVariables, x_reconst_ae = self.buildAutoencoder(x_raw)
        ...
        with tf.Session() as sess:
            ...
            if modelPath== '':
                for epoch in range(pretrainEpochs):
                    ...
                    for i in range(nValidBatches):
                        ...
                        x_reconst_, loss = sess.run([x_reconst_ae, loss_ae], feed_dict={x_raw:batchX})
                        # print(batchX[0, :20])
                        # print(x_reconst_[0, :20])
            ...
            for epoch in range(nEpochs):
                ...
                for i in range(nBatches):
                    ...
                    x_reconst_test, loss_ae_test = sess.run([x_reconst_ae, loss_ae], feed_dict={x_raw: batchX})
                    ...
                    a_loss_vec.append(loss_ae_test)
                buf = 'Epoch:%d, d_loss:%f, g_loss:%f, a_loss:%f, accuracy:%f, AUC:%f' % (epoch, np.mean(d_loss_vec), np.mean(g_loss_vec), np.mean(a_loss_vec), np.mean(validAccVec), np.mean(validAucVec))
                print(buf)
                self.print2file(buf, logFile)
...
def parse_arguments(parser):
    parser.add_argument('--embed_size', type=int, nargs="?", default=128, help='The dimension size of the embedding, which will be generated by the generator. (default value: 128)')
    parser.add_argument('--noise_size', type=int, nargs="?", default=128, help='The dimension size of the random noise, on which the generator is conditioned. (default value: 128)')
    parser.add_argument('--generator_size', type=tuple, nargs="?", default=(128, 128), help='The dimension size of the generator. Note that another layer of size "--embed_size" is always added. (default value: (128, 128))')
    parser.add_argument('--discriminator_size', type=tuple, nargs="?", default=(256, 128, 1), help='The dimension size of the discriminator. (default value: (256, 128, 1))')
    parser.add_argument('--compressor_size', type=tuple, nargs="?", default=(), help='The dimension size of the encoder of the autoencoder. Note that another layer of size "--embed_size" is always added. Therefore this can be a blank tuple. (default value: ())')
    parser.add_argument('--decompressor_size', type=tuple, nargs="?", default=(), help='The dimension size of the decoder of the autoencoder. Note that another layer, whose size is equal to the dimension of the <patient_matrix>, is always added. Therefore this can be a blank tuple. (default value: ())')
    parser.add_argument('--data_type', type=str, nargs="?", default='binary', choices=['binary', 'count'], help='The input data type. The <patient matrix> could either contain binary values or count values. (default value: "binary")')
    parser.add_argument('--batchnorm_decay', type=float, nargs="?", default=0.99, help='Decay value for the moving average used in Batch Normalization. (default value: 0.99)')
    parser.add_argument('--L2', type=float, nargs="?", default=0.001, help='L2 regularization coefficient for all weights. (default value: 0.001)')

    parser.add_argument('data_file', type=str, nargs="?", default=os.path.expanduser('~/data/PhisioNet/MIMIC/processed/out_binary.matrix'), metavar='<patient_matrix>', help='The path to the numpy matrix containing aggregated patient records.')
    parser.add_argument('out_file', type=str, nargs="?", default='/home/sina/experiments/medgan/models/', metavar='<out_file>', help='The path to the output file.')
    parser.add_argument('--model_file', type=str, nargs="?", default='/home/sina/experiments/medgan/models/-999', metavar='<model_file>', help='The path to the model file, in case you want to continue training. (default value: '')')
    parser.add_argument('--n_pretrain_epoch', type=int, nargs="?", default=100, help='The number of epochs to pre-train the autoencoder. (default value: 100)')
    parser.add_argument('--n_epoch', type=int, nargs="?", default=1000, help='The number of epochs to train medGAN. (default value: 1000)')
    parser.add_argument('--n_discriminator_update', type=int, nargs="?", default=2, help='The number of times to update the discriminator per epoch. (default value: 2)')
    parser.add_argument('--n_generator_update', type=int, nargs="?", default=1, help='The number of times to update the generator per epoch. (default value: 1)')
    parser.add_argument('--pretrain_batch_size', type=int, nargs="?", default=100, help='The size of a single mini-batch for pre-training the autoencoder. (default value: 100)')
    parser.add_argument('--batch_size', type=int, nargs="?", default=1000, help='The size of a single mini-batch for training medGAN. (default value: 1000)')
    parser.add_argument('--save_max_keep', type=int, nargs="?", default=0, help='The number of models to keep. Setting this to 0 will save models for every epoch. (default value: 0)')
    parser.add_argument('--generate_data', type=str2bool, nargs="?", default=False, help='If True the model generates data, if False the model is trained (default value: False)')
    parser.add_argument('--evaluate', type=str2bool, nargs="?", default=True, help='If True the model generates data, if False the model is trained (default value: False)')
...
if __name__ == '__main__':
    ...
    if not args.generate_data and not args.evaluate:
    # Training
    ...
    elif args.generate_data and not args.evaluate:
    ...
    # You must specify "--model_file" and "<out_file>" to generate synthetic data.
        ...
        # Load real data
        real_samples = np.load(args.data_file, allow_pickle=True)[:10000]

        # Load synthetic data
        gen_samples = np.load('out.npy', allow_pickle=True)
