...
class Model(BaseModel):
    def __init__(self, config):
        ...
        self.discriminatorNetLastLayer = tf.keras.Sequential(
            [
                tf.keras.layers.Conv2D(
    ...
    def discriminatorForward(self, x_dis):
        ...
        logits = self.discriminatorNetLastLayer(interfeatures)
        logits = tf.squeeze(logits, axis=[2, 3])
        interfeatures = tf.squeeze(interfeatures, axis=[2, 3])
    ...
    def build_discriminator(self):
        ...
        self.output_disc_input_real, interfeatures = self.discriminatorForward(x_real)
        self.output_disc_input_fake, interfeatures = self.discriminatorForward(x_fake)
        #####################
        ...
        with tf.control_dependencies(update_ops):
            ...
            gradients_of_discriminator = optimizer_discriminator.compute_gradients(self.disloss,
                                                                                   self.discriminatorNet.trainable_variables+self.discriminatorNetLastLayer.trainable_variables)
    ...
    def build_generator(self):
        ...
        # Decoding continuously generated values to their equivalent discrete values using the autoencoder.
        decoded_g, recounstructed_g = self.autoencoderDecode(generated)
        self.decoded_g = tf.squeeze(decoded_g, axis=[2, 3])
        x_real = self.expand_dim(self.x_real)

        # Shape match
        assert self.x_real.get_shape()[1:] == self.decoded_g.get_shape()[1:]

        # Labels generated by discriminator
        output_disc_input_fake_for_generator_loss, interfeatures_fake = self.discriminatorForward(decoded_g)
        output_disc_input_real_for_generator_loss, interfeatures_real = self.discriminatorForward(x_real)

        # Gen loss
        if self.config.feature_matching:
            logits, intermediate_output_input_real = self.discriminatorForward(x_real)
            logits, intermediate_output_input_generator_output = self.discriminatorForward(decoded_g)
