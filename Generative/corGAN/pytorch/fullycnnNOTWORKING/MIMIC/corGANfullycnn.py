...
parser.add_argument("--n_epochs_pretrain", type=int, default=1000,
...
parser.add_argument("--PATH", type=str, default=os.path.expanduser('~/experiments/pytorch/model/'+experimentName),
...
if not os.path.exists(opt.PATH):
    os.system('mkdir {0}'.format(opt.PATH))
...
# ave synthetic data
np.save(os.path.join(opt.PATH, "dataTrain.npy"), trainData, allow_pickle=False)
np.save(os.path.join(opt.PATH, "dataTest.npy"), testData, allow_pickle=False)
...
if opt.training:
        ...
        for i, samples in enumerate(dataloader_train):
            ...
            if batches_done % opt.sample_interval == 0:
                print(
                    ...
                    , flush=True)
                print('real_samples :', real_samples[0,:20])
                print('recons_samples :', recons_samples[0,:20])
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, samples in enumerate(dataloader_train):
            ...
            z = torch.randn((samples.shape[0], opt.latent_dim, 1), device=device)
        ...
        with torch.no_grad():
            ...
            fake_samples_test = torch.squeeze(autoencoderDecoder(fake_samples_test.unsqueeze(dim=2)))
...
if opt.generate:
    ...
    # Loading the checkpoint
    checkpoint = torch.load(os.path.join(opt.PATH, "model_epoch_500.pth"))
    ...
    for i in range(n_batches):
        ...
        z = torch.randn((opt.batch_size, opt.latent_dim, 1), device=device)
...
if opt.evaluate:
    # Load synthetic data
    gen_samples = np.load(os.path.join(opt.PATH, "synthetic.npy"), allow_pickle=True)
    ...
    p1 = plt.scatter(prob_real, prob_syn, c="b", alpha=0.5, label="corGAN")
